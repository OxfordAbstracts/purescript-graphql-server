...
import Data.Argonaut (Json, encodeJson, jsonNull, stringify)
...
toResponse (GqlResM gqlResM) = do
  ...
  case e of
    Left (ParseGqlDocumentError err) -> badRequest' $ "Failed to parse graphql document:\n" <> parseErrorMessage err
  ...
  where
  badRequest' error = badRequest $ stringify $ encodeJson
    { errors: Array.singleton error
    , data: jsonNull
    }
